...
7. **公文排版与字体自动化**（v2.1）：内置集团《行文规范性格式模板》全套规则（方正小标宋二号主标题、仿宋_GB2312三号正文、28磅固定行距、A4公文页边距、外侧奇偶页码）；表格一律仿宋_GB2312五号、表头首行加粗、宽度按窗口自动调整、所有单元格居中；三个公文字体随技能分发，`scripts/ensure-fonts.ps1` 自动检测——本地已装直接用，缺失则用户级安装（无需管理员权限），assets 缺失时自动从本仓库下载。
