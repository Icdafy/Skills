...
| 交付物 | 说明 |
| --- | --- |
| 章节正文 | 骨架与必要表格参照范文，正文按事实关系自然行文（两阶段分别保留原报告固定标题，历史事项均按事实逐项展开）；默认 Markdown，可一键渲染公文排版 .docx |
...
技能先等待用户选择阶段，再完成：建事实台账 → 榨取会议纪要 → 联网补缺与背景增强 → 公式勾稽＋十四对三角验证 → 按骨架起草 → 红旗扫描 → 语言红线扫描 → 机械勾稽自检 → 交付。

## 章节标题

早前期使用 2026-09 终稿范式第四章（设立背景开篇，28个源标题；伊隆纬特结构已停用）；中后期使用因诺原第二章和第五章，财务第六章不并入。所有子标题以两份完整对照及JSON为准。
...
| `references/data-and-tables.md` | 计算公式、口径与单位、勾稽自检清单、联网三大必检、标准表模板 T1–T21 |
...
- 此处为历史版本记录；当前已升级为先询问阶段、按原报告固定模板填空。旧四节制及自动增配规则停用。
...
- 早前期模板改为终稿结构（28个源标题）：设立背景开篇，新增关联公司及经营主体、资质及荣誉、本轮融资及资金使用计划、公司收入预测及订单依据（已签、意向）；历史股权变动不再逐次设标题，财务问题并入核心财务数据分析段；
- `structure.md` 新增“早前期终稿范式逐节写法”与改写规律；新增表模板 T18—T21；
- 三技能共用的投资逻辑复核新增全报告逻辑主线、论据强度分级与简明公文表达；
- `style_check.py` 新增方法论/告诫句与内部资料编号扫描；`build_docx.py` 段落块支持 `lead`。
