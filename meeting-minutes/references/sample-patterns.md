...
- Metadata: `访谈时间`, `访谈地点`, `访谈人员`; sometimes `同行人员`, `被访谈人`, `访谈对象`, or `访谈人员名单`.
- Participant list: often a table with `姓名 / 性别 / 单位 / 备注` or `姓名 / 性别 / 单位 / 职务 / 备注`.
- Lead paragraph: longer due-diligence samples use `本次访谈围绕...展开，现将...整理如下：`.

Common body structures:

- Short project interviews: `一、访谈人员名单` + `二、主要内容`, with second-level sections for company, product, market, production, finance, cooperation.
- Deep due-diligence interviews: topic-specific first-level sections such as background, organization, business segments, technology, competition, finance, equity, listing plan, risks.
...
- Follow-up/action-oriented meetings: end with `后续行动计划`, `待办事项清单`, or `待核实事项`.
...
- `方元明鑫访谈纪要12.9.docx`: classic compact template: participant list, `主要内容`, then financing, products, customers, production, team, operations, advantages, future planning.
...
- `访谈纪要（西安超验）-海老师.docx`: more detailed technical interview: background/team, technology/product advantages, market/competition, production/commercialization, qualification/risk.
...
- `访谈纪要-伊隆纬特-4.24.26.docx` and `伊隆维特访谈纪要.docx`: communications project memo; useful sections include technology route, advantages, market layout, milestones, financing, risk.
...
- `华岳访谈纪要-香兰素专家.docx`: expert interview with summary and investment suggestion; keep expert claims attributed and separate from memo conclusions.
- `华岳访谈纪要-销售总监.docx`: role-specific interview; sections follow personal background, motivation, work plan, Q&A.
- `华岳生物访谈.docx`: concise synthesized memo without full front matter; uses industry background, company, production/operations, market/cooperation, financing, goals/risks.
...
- `空天动力电推进器线上会议纪要4.1.26.docx`: online meeting memo; uses `会议时间/会议地点`; ends with action items and investment/financing follow-up.
...
- `液氢动力项目情况及发展规划讨论会议纪要.docx`: discussion meeting memo; useful ending structure: risks/challenges and follow-up action plan.
...
- `因诺科技` role interviews: deep due-diligence template. Each interview focuses on one function: chairman/company strategy, defense business, finance, R&D, HR/supply chain. Use role-specific sections and preserve operational figures, payback periods, margins, headcount, order values, and action items.
...
- If it contains explicit tasks/responsible parties/time nodes, add `待办事项清单`.
...
- If it is voice transcription without headings, infer headings from recurring topics and use `待核实事项` for unclear metadata.
...
- `待核实事项`: use when the transcript lacks data or contains contradictions.
