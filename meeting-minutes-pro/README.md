...
| 双引擎定向复核 | `refine_transcript.py` 自动挑出含数字、日期、术语、提问的高风险片段，用第二引擎重转并按金额/百分比/日期/否定词/术语五类比对，分歧片段必须回听；数字比对顺序敏感防张冠李戴，`--budget-minutes` 风险预算适配低配机器（最高风险片段必选），分歧片段自动剪出回听音频 |
