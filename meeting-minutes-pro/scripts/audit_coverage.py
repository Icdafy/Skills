...
class Window:
    def __init__(self, index: int, label: str, text: str) -> None:
        self.index = index
        self.label = label
        self.text = text
...
def time_windows(stamps: list[dict], window_seconds: float) -> list[Window]:
    ...
    while start < duration:
        ...
        windows.append(Window(index, label, "".join(members)))
        start = end
    return windows


def text_windows(text: str, chars_per_window: int) -> list[Window]:
...
def build_windows(path: Path, window_seconds: float, chars_per_window: int) -> tuple[list[Window], str]:
    stamps, text = load_transcript(path)
    if stamps:
        return time_windows(stamps, window_seconds), "time"
    if not text.strip():
        raise SystemExit(f"转录稿为空：{path}")
    return text_windows(text, chars_per_window), "text"
...
def validate(
    ...
    minutes_text = fact_check.normalize(minutes_path.read_text(encoding="utf-8-sig"))
...
def main() -> int:
    ...
    windows, mode = build_windows(args.transcript, args.window_seconds, args.chars_per_window)
