...
3. 判定输出模式。整场以问答为主（存在显式“问/答”“Q/A”标记，或问答组占实质内容多数）时使用问答纪要；以陈述讨论为主但存在任何明确问答对时使用混合式纪要（主题式正文 + 末尾“问答环节”章节，或将零星问答就地嵌入所属议题）；完全没有问答时使用主题式纪要。**检测到任何一组明确的问答都必须以“问：/答：”结构保留，不得被摘要性叙述吞并。**
4. 长转录稿分两遍处理。第一遍按主题或时间分段起草：每段先列出该段全部数字、人名、日期、承诺和结论的要点清单，再归纳成正文。第二遍对照审计：拿着完成的草稿逐句回到原始转录稿核对——① 每个数字、金额、百分比、日期能否在转录稿中找到原文；② 每个“会议明确”“会议同意”等结论性表述在转录稿中是否有明确表态依据；③ 将转录稿按 5–10 分钟分窗，逐窗确认“已纳入纪要第 X 节”或“寒暄/重复，判定省略”，任何一个时间窗都不得没有判定。审计发现问题即修改，改后重审。
...
- `references/format-and-output.md`：固定字体、版式、层级、写作口径和三种纪要模板（主题式、问答式、混合式）。
