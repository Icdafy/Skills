...
4. 长转录稿分两遍处理。第一遍禁止直接起草摘要：按时间窗逐段列出全部量化事实，并为每项记录“时间戳＋对象＋数值/区间＋单位＋时间范围＋限定词＋比较基准＋说话人口径”。先对全量双引擎差异逐项回听，补齐主转录稿，再完成全部问答整理和主题化总结。第二遍对照审计：拿着完成的草稿逐句回到原始转录稿核对——① 每项量化事实及其对象、单位、范围和限定词是否完整进入纪要；② 每个“会议明确”“会议同意”等结论性表述是否有明确表态依据；③ 每个时间窗的重要内容是否进入总结或问答；④ 受访人自述、管理层口径和核查结论是否明确区分。审计发现问题即修改，改后重审。
