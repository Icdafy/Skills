...
6. 用户明确要求跳过提问、仅需原始转录，或表示相关信息不清楚时，不阻塞后续处理。未知字段不得猜测；按输出规则直接省略无依据的字段，不在纪要中标注“待核”等字样。
7. 用户补充信息与转录内容明显冲突时，只针对冲突字段简短核对。无法核实时保留用户明确提供的口径，并在对话中向用户说明冲突；不得擅自合并两个不同口径，也不得在纪要正文中作“待核”标注。
