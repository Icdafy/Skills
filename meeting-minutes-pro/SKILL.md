---
name: meeting-minutes-pro
description: 在本地将中文、方言、英文或多语言会议录音和视频转录为文字（默认 FunASR Paraformer 引擎，支持数小时长音频、说话人分离和热词；多语言场景可切换 Qwen3-ASR 引擎），并将音视频、转录稿、访谈记录、尽调问答、路演记录和会议笔记整理为完整、客观、书面化的正式会议纪要；在生成纪要前主动收集缺失的会议或访谈时间、地点、对象、人员等基本信息；自动识别问答内容，对尽调、访谈、路演和项目交流默认生成“总结研判＋完整问答”组合纪要，同时保留纯问答和主题式模式；交付前对纪要中的每一个数字执行事实核对，确保与转录稿一致。适用于需要本地私密转录、长录音处理、会议信息采集、会议内容完整总结、核心结论与风险提炼、数字零差错核验、国有企业公文格式排版、会议纪要 DOCX 输出和 QA 问答整理的场景。
---

# Meeting Minutes Pro

在用户设备上运行本地 ASR 引擎，录音文件保持本地处理。将转录稿作为事实基础，形成可直接流转的会议纪要。默认交付经整理并通过格式校验、事实核对的纪要正文；用户明确只需要原始转录时，完成转录核验后结束。

## 前置信息采集

1. 用户要求生成会议纪要、访谈纪要、问答纪要或 DOCX 时，先检查当前对话、文件名和用户已提供的说明，提取已有基本信息，不重复询问已明确的字段。
2. 检查 `<skill-dir>/glossary/` 下是否已有同一项目或公司的术语文件（如 `glossary/某某公司.txt`）；存在时直接读取并入热词，不再要求用户重复提供。
3. 在开始完整转录或纪要起草前，一次性询问仍缺失的基本信息并等待用户回答。根据会议类型从以下字段中选择适用项：
   - 会议、项目或公司名称；
   - 会议或访谈时间；
   - 会议或访谈地点、线上会议平台；
   - 访谈对象或主要参会人员的姓名、单位和职务；
   - 访谈人员或纪要所需的我方参会人员；
   - 核心议题，以及需要辅助识别的人名、简称和专业术语。
4. 使用简短表单集中提问，例如：

```text
在转录并生成正式纪要前，请补充以下尚缺信息；不知道的项目可填写“不清楚”或“跳过”：
1. 访谈时间：
2. 访谈地点/会议平台：
3. 访谈对象（姓名、单位、职务）：
4. 访谈人员（我方）：
5. 项目/公司名称及访谈主题：
6. 需要重点识别的人名、简称或专业术语（可选）：
```

5. 用户回答后直接继续既定任务，不再次要求确认。将用户补充及术语文件中的人名、项目名称、简称和术语传入转录命令的 `--context`，并将确认的基本信息写入最终纪要正文和 DOCX。任务结束前，把本次新出现的人名、简称和术语追加进对应的 `glossary/<项目或公司名>.txt`，供同一项目后续会议复用。
6. 用户明确要求跳过提问、仅需原始转录，或表示相关信息不清楚时，不阻塞后续处理。未知字段不得猜测；按输出规则省略无依据的字段或在确需展示时标注“待核”。
7. 用户补充信息与转录内容明显冲突时，只针对冲突字段简短核对。无法核实时保留用户明确提供的口径，并在对应位置标注“待核”，不得擅自合并两个不同口径。

## 输入分流

1. 收到音频或视频且用户要求生成纪要时，先完成“前置信息采集”，再执行“本地转录流程”，保存原始转录文件后进入“会议纪要流程”；用户仅需原始转录时直接执行“本地转录流程”。
2. 收到现成转录稿、会议笔记或访谈记录时，完成适用的“前置信息采集”后进入“会议纪要流程”。
3. 用户要求 TXT、Markdown、JSON、SRT 或仅需语音转文字时，按转录输出要求交付；用户要求纪要、总结、访谈纪要或问答整理时，最终成果必须是整理后的纪要正文。

## 本地转录流程

1. 将媒体文件解析为绝对路径，核实文件存在并识别格式、大小和时长。
2. 选择引擎：中文或中英混杂的会议、访谈、路演一律使用默认的 `funasr` 引擎（Paraformer 流水线，原生支持数小时长音频、句级时间戳、热词和说话人分离）；纯外语、粤语等方言或多语言混合录音使用 `--engine qwen`（Qwen3-ASR，支持 52 种语言，长音频自动按静音切块并断点续传）。
3. 检查运行环境：

```powershell
python <skill-dir>/scripts/bootstrap_runtime.py --check
```

4. 首次运行缺少依赖或模型时，先取得用户对网络下载的许可，再安装运行环境（`--engine funasr`、`--engine qwen` 或 `--engine all`）：

```powershell
python <skill-dir>/scripts/bootstrap_runtime.py --install --engine funasr
```

5. 对时长较长、内容重要或噪声较多的录音，先转录 60 秒样本核对效果；录音噪声大、音量低时加 `--enhance`：

```powershell
<runtime-python> <skill-dir>/scripts/transcribe.py --input <media-path> --output-dir <output-dir> --clip-duration 60
```

6. 核对样本后执行完整转录。访谈、尽调、路演和任何可能含问答的多人会议加 `--diarize` 获取说话人轮次；通过 `--context` 提供人名、项目名称、简称、专业术语和数字写法（FunASR 引擎作为热词，Qwen 引擎作为上下文）：

```powershell
<runtime-python> <skill-dir>/scripts/transcribe.py --input <media-path> --output-dir <output-dir> --diarize --context "人名 公司名 专业术语"
```

7. 长音频转录期间脚本会在 stderr 输出 JSON 进度；qwen 引擎的分块结果保存在输出目录 `<文件名>.chunks/` 中，中断后重新执行同一命令即自动续传。
8. 需要 SRT 或带时间戳文本时加 `--timestamps`（funasr 引擎句级时间戳无额外开销；qwen 引擎会下载并加载 Qwen3-ForcedAligner-0.6B，内存占用较高）。
9. 对数字密集、结论重大的关键录音，可执行双引擎交叉核验：分别用 `funasr` 和 `qwen` 各转录一遍，再运行 `fact_check.py --compare 转录A.txt 转录B.txt`；仅出现在单一转录稿中的数字，写入纪要时必须标注“待核”或回听录音确认。
10. 核对生成文件、首条和末条非空转录内容，说明实际引擎、模型、设备、识别语言、输出位置和可能影响准确率的因素。

## 会议纪要流程

1. 通读并清理转录稿。修正明显断句和可确认的同音字；人名、金额、日期、项目名称、文件名称和专业术语无法核实时保留原文或标注“待核”。启用了 `--diarize` 时，结合前置信息采集确认的参会人名单，将“说话人N”映射到真实姓名与角色；没有把握时保留“说话人N”编号，不得猜测。
2. 合并用户在“前置信息采集”中确认的内容与材料中明确出现的信息，提取会议名称、时间、地点、访谈对象、访谈人员、参会范围、议题、事实、意见、结论、任务、责任人和时限。缺失信息不补写；两类来源发生冲突时按前置信息采集规则处理。
3. 判定输出模式：
   - 尽调、访谈、路演或项目交流以问答为主时，默认使用**组合式问答纪要**：基本信息之后先写“核心结论摘要”，提炼总体判断、核心事实、技术或业务判断、主要风险和后续核查事项；随后设置“完整问答纪要”，逐组保留全部实质问答。
   - 用户明确要求“只要问答”“保持纯 QA”或材料只是一般答疑记录时，使用纯问答纪要。
   - 以陈述讨论为主但存在明确问答对时，使用混合式纪要（主题式正文 + 末尾“问答环节”，或将零星问答就地嵌入所属议题）。
   - 完全没有问答时，使用主题式纪要。
   - **检测到任何一组明确问答都必须以“问：/答：”结构保留；摘要只能增加阅读入口，不得替代、删减或吞并完整问答。**
4. 长转录稿分两遍处理。第一遍按主题或时间分段起草：每段先列出该段全部数字、人名、日期、承诺和结论的要点清单，再归纳成正文。组合式问答纪要的摘要应在完整问答起草完成后提炼，每个摘要判断都要能回溯到一个或多个问答，不增加材料之外的投资结论。第二遍对照审计：拿着完成的草稿逐句回到原始转录稿核对——① 每个数字、金额、百分比、日期能否在转录稿中找到原文；② 每个“会议明确”“会议同意”等结论性表述在转录稿中是否有明确表态依据；③ 摘要中的每项判断是否与完整问答一致，受访人自述、管理层口径和核查结论是否明确区分；④ 将转录稿按 5–10 分钟分窗，逐窗确认“已纳入纪要第 X 节”或“寒暄/重复，判定省略”，任何一个时间窗都不得没有判定。审计发现问题即修改，改后重审。
5. 读取 `references/format-and-output.md`，按固定公文格式组织正文。先保证内容完整，再压缩重复口语、寒暄和无实质内容的插话。
6. 保留影响会议结论、任务安排、时间节点、争议焦点、条件限制和问答实质的信息。各方意见存在差异时，分别记录其范围和依据。
7. 交付前依次执行格式校验和数字事实核对：

```powershell
python <skill-dir>/scripts/quality_check.py <minutes-text-file> --mode <auto|minutes|qa|qa-summary>
python <skill-dir>/scripts/fact_check.py <minutes-text-file> --transcript <transcript-txt>
```

8. `fact_check.py` 列出的每一个无依据数字必须处理后重跑直至通过：改回转录稿原文、标注“待核”，或确属用户提供/合理推定（如由“去年”推出的年份）时用 `--allow <数字>` 显式放行并向用户说明。
9. 修复脚本提示，并人工复核事实完整性、层级顺序、两字缩进、客观语气和问答配对。

## 写作口径

- 使用客观、审慎、书面化表述，聚焦会议所述事实、意见和处理安排。没有来源依据的判断、评价、原因、结论、责任人和期限均不写入。
- 完整归纳会议内容，不逐句复述。保留不同意见及其边界，不将讨论倾向写成已形成的决定。
- 仅在材料明确显示时使用“会议明确”“会议同意”“会议要求”等结论性措辞；讨论中的建议使用“提出”“建议”“表示”等中性动词。
- 不虚构参会人员、发言人、职责、任务分工或截止时间。没有可靠说话人标识时不推断发言主体。
- 问答内容按最高保真级别处理：问题的指向不得改变，答复中的数字、口径、条件、承诺和时间节点不得合并、稀释或改写。
- 组合式问答纪要的摘要只提炼问答中已有的信息。将受访人观点写为“受访人表示/认为”，将未经验证的数据写为“据其自述”，将外部核查后才能成立的事项列入“风险与待核事项”；不得把摘要写成无依据的投决结论。
- 删除宣传性语言、主观评价、泛化意义阐释和机械的结尾展望。避免用户指定的对照式连词组合，改用直接陈述。
- 数字、时间、单位、文件名称和责任主体照录或谨慎规范化；出现歧义时标注“待核”。

## 输出结构

默认以纯文本提供纪要正文，标题外的一级至四级标题及全部段落均以两个全角空格起首。

- 主题式纪要：标题；材料中明确的会议基本信息；按议题展开的事项、意见、结论和工作安排。
- 组合式问答纪要（尽调、访谈、路演和项目交流的默认问答模式）：标题；材料中明确的基本信息；“一、核心结论摘要”，至少覆盖总体判断、核心事实或能力、主要风险与待核事项；“二、完整问答纪要”，按主题逐组列出“问：”和“答：”。摘要不得代替后文问答。
- 纯问答纪要：标题；材料中明确的会议基本信息；“一、问答纪要”下按主题列出“问：”和“答：”。仅在用户明确要求纯 QA 或材料属于一般答疑记录时使用。
- 混合式纪要：主题式正文在前；集中出现的问答（路演答疑、访谈问答环节等）在正文最后单设一章“X、问答环节”，章内按主题分组列出“问：/答：”；散落在个别议题中的零星问答就地保留在该议题之下，仍用“问：/答：”结构。
- 访谈、尽调和项目交流纪要的基本信息固定按 `访谈时间`、`访谈地点`、`访谈对象`、`访谈人员` 的顺序排列。用户在前置信息采集中确认的内容属于有效依据；没有依据的字段直接省略，不改变其余字段的相对顺序。
- 没有材料依据的基本信息不设置空白占位段。确需核验的内容在对应位置标注“待核”。

## 交付物归档

同一录音的全部产物集中放在一个以录音文件名命名的目录中，转录稿是纪要的证据链，必须与纪要一同保留：

```text
<录音文件名>/
  transcript.txt / .md / .json（/.srt）   原始转录稿
  会议纪要.txt                              通过校验的纪要正文
  会议纪要.docx                             正式交付文件
```

正式交付物只有 DOCX。渲染检查生成的 PDF 属于内部校验产物，逐页核对完成后删除，不随纪要交付，用户明确索要 PDF 时除外。

## DOCX 交付

先通过文本校验和事实核对，再检查固定版式所需字体：

```powershell
<runtime-python> <skill-dir>/scripts/font_preflight.py --check
```

缺少随技能提供的字体时，说明安装会修改当前用户字体目录；取得用户许可后运行 `<runtime-python> <skill-dir>/scripts/font_preflight.py --install-user`。缺少黑体或宋体时提示用户安装，不在字体替换状态下继续生成。字体检查通过后运行；以下为组合式问答纪要示例，其他输出分别使用 `minutes`、`qa` 或 `auto`：

```powershell
<runtime-python> <skill-dir>/scripts/create_minutes_docx.py `
  --input "会议纪要.txt" `
  --output "会议纪要.docx" `
  --subtitle "综合办公室" `
  --mode qa-summary
```

脚本会再次执行文本校验，并依据公司样例设置 A4 页面、页边距、字体、字号、固定行距、两字缩进和奇偶页外侧页码。生成后运行渲染检查：

```powershell
<runtime-python> <skill-dir>/scripts/render_docx.py --input "会议纪要.docx"
```

逐页检查生成的 PDF：字体替换（结果中 `missing_required_fonts` 或 `substituted_fonts` 非空即不得交付；标题方正小标宋因许可禁止嵌入、以轮廓输出属正常，须目检字形）、标题偏移、段落截断、异常分页和页码位置均需修正后再次生成并渲染。检查全部通过后删除该 PDF，仅交付 DOCX。本机既无 Microsoft Word 也无 LibreOffice 时，向用户说明无法本机渲染，请用户自行打开 DOCX 核对版式。

## 转录与隐私规则

- 默认引擎为 FunASR（`paraformer-zh` + `fsmn-vad` + `ct-punc`，可选 `cam++` 说话人分离），模型从 ModelScope 下载；备选引擎为 Qwen3-ASR（默认 `Qwen/Qwen3-ASR-0.6B`，官方 `qwen-asr` Transformers 后端），模型从 Hugging Face 下载。
- 自动优先使用 CUDA；无可用 CUDA 时使用 CPU。Apple 设备默认使用 CPU，用户可用 `--device mps` 显式尝试。检测到可用 CUDA 且用户对精度要求高时，可询问用户是否改用 `--model Qwen/Qwen3-ASR-1.7B`（qwen 引擎），不静默替换。
- 运行环境和模型已缓存后，可使用 `--offline` 禁止网络访问。
- 默认生成 `.txt`、`.md` 和 `.json`；有句级时间戳时增加 `.srt`；启用 `--diarize` 时 `.md`、`.json` 和 `.srt` 中带说话人标签。
- 不上传录音，不静默切换云端接口，不虚构说话人标签。噪声、重叠发言、音乐、削波或弱语音可能影响准确率时，应明确提示。

## 资源

- `references/format-and-output.md`：固定字体、版式、层级、写作口径和四种纪要模板（主题式、组合式问答、纯问答、混合式）。
- `references/runtime.md`：双引擎选择、硬件、下载、隐私、离线运行和故障处理说明。
- `references/platforms.md`：在不同智能体平台安装和分发技能的说明。
- `scripts/bootstrap_runtime.py`：检查和安装本地转录运行环境（`--engine funasr|qwen|all`）。
- `scripts/transcribe.py`：执行本地音视频转录（FunASR/Qwen 双引擎、长音频切分、断点续传、说话人分离、音频增强）。
- `scripts/install_skill.py`：安装和分发技能。
- `scripts/requirements-runtime.txt`、`requirements-funasr.txt`、`requirements-qwen.txt`：基础与分引擎运行依赖。
- `scripts/quality_check.py`：检查缩进、标题层级、访谈基本信息顺序、问答配对和禁用表达。
- `scripts/fact_check.py`：核对纪要中的数字与转录稿一致（支持中文数字、万/亿换算、百分比），并支持双转录稿交叉核验。
- `scripts/create_minutes_docx.py`：以固定公文版式生成 DOCX。
- `scripts/render_docx.py`：将 DOCX 渲染为 PDF，报告页数与实际内嵌字体，用于交付前逐页检查。
- `scripts/font_preflight.py`：检查固定版式所需字体，并在用户许可后安装随技能提供的字体。
- `glossary/`：按项目或公司维护的热词术语文件，跨会议复用。
- `assets/fonts/`：方正小标宋简体、楷体_GB2312、仿宋_GB2312 字体文件。
- `assets/templates/文件字体格式.doc`：公司格式样例。发生格式冲突时，以样例和 `references/format-and-output.md` 为准。
