...
description: 使用本地 Qwen3-ASR-0.6B 将中文、方言、英文或多语言会议录音和视频转录为文字，并将音视频、转录稿、访谈记录、尽调问答、路演记录和会议笔记整理为完整、客观、书面化的正式会议纪要；在生成纪要前主动收集缺失的会议或访谈时间、地点、对象、人员等基本信息，自动识别连续问答并转换为问答纪要。适用于需要本地私密转录、会议信息采集、会议内容完整总结、国有企业公文格式排版、会议纪要 DOCX 输出和 QA 问答整理的场景。
---
...
2. 在开始完整转录或纪要起草前，一次性询问仍缺失的基本信息并等待用户回答。根据会议类型从以下字段中选择适用项：
   - 会议、项目或公司名称；
   - 会议或访谈时间；
   - 会议或访谈地点、线上会议平台；
...
4. 用户回答后直接继续既定任务，不再次要求确认。将用户补充的人名、项目名称、简称和术语传入转录命令的 `--context`，并将确认的基本信息写入最终纪要正文和 DOCX。
5. 用户明确要求跳过提问、仅需原始转录，或表示相关信息不清楚时，不阻塞后续处理。未知字段不得猜测；按输出规则省略无依据的字段或在确需展示时标注“待核”。
6. 用户补充信息与转录内容明显冲突时，只针对冲突字段简短核对。无法核实时保留用户明确提供的口径，并在对应位置标注“待核”，不得擅自合并两个不同口径。
