...
两项一级标题和第一部分的前三项二级标题（存续基金、新设基金、参股公司）固定不变。SPV 位置是可重复位置：数量由来源模板决定，最少一个，最多至 `（十）`，且序号必须自 `（四）` 起连续。将来源原文依次写入 `document.source_fixed_main_headings`，将最终序列写入 `document.fixed_main_headings`，两份列表长度可以是 6 至 12 项。`document.heading_contract_source` 必须非空，用于记录所检查的基础模板来源，但校验器不会据此自动认证模板真实性。`document.heading_change_authorized` 必须是布尔值，`document.heading_change_note` 必须始终非空。

两份标题列表一致时，授权值必须为 `false`，备注应说明未调整固定标题。只有 SPV 位置可以在用户后续回答确认项目变化后调整：可以改写某个位置的项目名称，也可以按新增或退出的 SPV 项目增减位置；此时授权值必须为 `true`，备注须逐项写明来源标题和输出标题。两项一级标题和前三项二级标题在现行校验器下不得变化。不得因公开示例使用通用名称，就擅自把来源特定 SPV 标题通用化。
...
固定一级标题 `一、年度股权投资完成总体情况` 中的“年度”属于不可变的模板措辞，半年度和季度报告同样保留，不随 `report_period` 改写；如需改动，须先更新固定标题契约。
