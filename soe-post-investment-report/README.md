...
- 正文沿用来源模板派生的固定框架：第一项一级标题写作 `一、<报告期间>股权投资完成总体情况`（随报告期间变化），`二、重大投资项目进展情况` 与前三项二级标题固定，SPV 为可重复位置，自 `（四）` 起连续编号，按项目分设。
...
`document.source_fixed_main_headings` 记录来源模板标题，`document.fixed_main_headings` 记录输出标题，两份列表长度为 6 至 12 项。除用户确认后的 SPV 位置（可改名，也可按新增或退出的 SPV 项目增减数量）外，两者必须一致（第一项一级标题的期间措辞由 `report_period` 推导，不计入差异）；`（一）存续基金`、`（二）新设基金`、`（三）参股公司` 和 `二、重大投资项目进展情况` 在现行校验器中不可更改。SPV 位置须自 `（四）` 起连续编号，且每个具名位置在项目名册中有对应记录。
