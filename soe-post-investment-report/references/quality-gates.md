...
- [ ] `document.report_period` 已按数据截止日期确定；6月30日和12月31日的口径歧义已由用户确认。
- [ ] 大标题期间措辞由 `report_year` 与 `report_period` 生成，未沿用上期标题。
- [ ] 开头依据中的期间措辞与标题一致。
- [ ] 主送单位和开头依据齐全。
- [ ] `一、<报告期间>股权投资完成总体情况` 只出现一次，期间措辞与 `document.report_period` 一致。
- [ ] `二、重大投资项目进展情况` 只出现一次。
...
- [ ] 数据截止日期不晚于成文日期。
- [ ] 印发日期不早于成文日期。
- [ ] 正文中各项数据自身的口径与时点已写明；与报告期末不同的数据已单独标注截至日期。
